...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    dh1, dgamma, dbeta = batchnorm_backward(dh2, bn_cache)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    self.params['W' + str(M+1)] = weight_scale * np.random.randn(hidden_dims[-1], num_classes)
    self.params['b' + str(M+1)] = np.zeros(num_classes)
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers-1):
        if not self.use_batchnorm:
            ...
            scores, cache = affine_batchnorm_relu_forward(scores, self.params['W' + str(i+1)], self.params['b' + str(i+1)], 
    ...
    ############################################################################
    N = X.shape[0]
    C = self.params['W'+str(self.num_layers)].shape[1]
    ...
    dbetas = []
    caches.reverse()
    dscores, dw, db = affine_backward(dscores, caches[0])
    dws.append(dw)
    dbs.append(db)
    for c in caches[1:]:
        if self.use_dropout:
            dscores = dropout_backward(dscores, c[1])
            c = c[0]

        if self.use_batchnorm:
            dscores, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(dscores, c)
    ...
    for i in xrange(len(dws)):
        grads['W'+str(it)] = dws[i] / N + self.reg * self.params['W'+str(it)]
        grads['b'+str(it)] = dbs[i] / N
        if self.use_batchnorm and it > 1:
            grads['gamma'+str(it-1)] = dgammas[i] / N
            grads['beta'+str(it-1)] = dbetas[i] / N
        it -= 1
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
